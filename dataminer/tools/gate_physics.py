"""Run the deterministic Franka physical task gate and save its rollout."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("out/gate_sim.mp4"))
    ...
    print(
        f"trajectory {len(trajectory)} steps @30Hz "
    ...
    print(f"rollout {time.time() - started:.1f}s")
    print(episode.validation.report())
    write_video(
        args.output,

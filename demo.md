...
```bash
.venv/bin/python -m src.robot_data_job \
...
.venv/bin/python -m src.product_runloop \
  config/demo_request.json \
  --output-dir artifacts/runloop_demo
```

제품 백엔드는 위 명령으로 Devbox를 만들고, Reflex Agent에서는 Reflex가
이미 Devbox를 만들기 때문에 `src.product_request`를 직접 실행한다. Reflex에서는
`reflex/robot_data_agent_prompt.md`를 사용한다. MP4는 Reflex 첨부 제한 때문에
...
```bash
.venv/bin/python -m src.runpod_training_runner config/training_request.json
```
...
.venv/bin/python -m src.runpod_training_runner \

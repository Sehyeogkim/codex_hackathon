...
RUNTIME_PATHS = (
    "mimic",
    "src",
    "scripts/demo2_train_policy.py",
    "scripts/download_dexycb.sh",
...
def remote_training_command(request: TrainingRequest) -> str:
    ...
    body = (
        ...
        f"cd {root}; "
        "apt-get update -qq; "
        "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq libgles2 >/dev/null; "
...
def remote_dexycb_training_command(request: TrainingRequest) -> str:
    ...
    body = (
        ...
        "python scripts/demo2_train_policy.py \"${trajectory_args[@]}\" "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("request", type=Path, help="Training request JSON")

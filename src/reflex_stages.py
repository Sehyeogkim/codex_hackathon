...
Examples::
    ...
    python -m src.reflex_stages reconstruction --input demo.mp4 --output vision.json
    python -m src.reflex_stages retargeting --input vision.json \
        --config config/demo_config.json --output panda.json \
        --grasp-frame 30 --release-frame 70
    python -m src.reflex_stages validation --input panda.json \
        --config config/demo_config.json --output physics.json
    python -m src.reflex_stages scaling --input panda.json \
        --config config/demo_config.json --output scaled.json --count 20
"""
...
def _compile_scaling_seed(trajectory: np.ndarray, demo: Any) -> np.ndarray:
    ...
    from mimic.rollout import waypoints_to_traj
...
def _default_dependencies() -> StageDependencies:
    from mimic.augment import augment
    from mimic.config import DemoConfig
    from mimic.export import episode_record

    from .panda_sim import compile_trajectory
...
def run_scaling(
    ...
    *,
    ...
    ``count`` intentionally means attempts, matching :func:`mimic.augment.augment`.

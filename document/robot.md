...
![Franka Emika Panda](media/franka_panda.png)

## 선택 이유

- 7축 로봇팔과 평행 그리퍼를 사용해 병 pick-and-place에 적합하다.
- MuJoCo Menagerie 모델이 공개돼 IK와 물리 시뮬레이션을 재현할 수 있다.
...
- 태스크: 머스터드 병을 A 지점에서 B 지점으로 옮기기
- 입력: 사람이 같은 태스크를 수행하는 영상
- 출력: 7개 관절의 시간별 각도와 gripper 상태
- 검증: MuJoCo에서 trajectory 재생 및 충돌·관절 제한·태스크 성공 여부 확인
